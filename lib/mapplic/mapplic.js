	...
	var Mapplic = function() {
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				this.image = $('<img style="padding-top:10px;padding-left:5px;">').addClass('mapplic-tooltip-image').hide().appendTo(this.el);
				this.title = $('<h4></h4>').addClass('mapplic-tooltip-title').appendTo(this.el);
				this.content = $('<div></div>').addClass('mapplic-tooltip-content').appendTo(this.el);
				this.desc = $('<div></div>').addClass('mapplic-tooltip-description').appendTo(this.content);
				this.link = $('<a>Visit</a>').addClass('mapplic-tooltip-link').attr('href', '#').hide().appendTo(this.el);
		...
		function Sidebar() {
			...
			this.init = function() {
				...
				this.el = $('<div id="mapplic-search-input"></div>').addClass('mapplic-sidebar').appendTo(self.el);

				if (self.o.search) {
					var form = $('<form class="item item-input"></form>').addClass('mapplic-search-form').submit(function() {
						return false;
					}).appendTo(this.el);
					...
					var input = $('<input class="icon ion-search placeholder-icon">').attr({'type': 'text', 'spellcheck': 'false', 'placeholder': 'Search...'}).addClass('mapplic-search-input').keyup(function() {
